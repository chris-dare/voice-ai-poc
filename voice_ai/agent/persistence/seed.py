    ...
    async with database.session_factory.begin() as session:
        ...
        plans = (
            Plan(
                ...
                data_allowance_mb=5_120,
                voice_minutes=120,
                description="5 GB data and 120 voice minutes",
            ),
            Plan(
                code="SMART_35",
                name="Smart 35",
                monthly_price=Decimal("35.00"),
                data_allowance_mb=12_288,
                voice_minutes=300,
                description="12 GB data and 300 voice minutes",
            ),
            Plan(
                code="MAX_60",
                name="Max 60",
                monthly_price=Decimal("60.00"),
                data_allowance_mb=30_720,
                voice_minutes=1_000,
                description="30 GB data and 1,000 voice minutes",
            ),
        )
        for plan in plans:
        ...
        if reset or await session.get(DataUsage, primary.id) is None:
            await session.merge(
                DataUsage(
                    subscriber_id=primary.id,
                    cycle_start=today - timedelta(days=22),
                    cycle_end=today + timedelta(days=8),
                    used_mb=9_216,
                )
            )
        ...
        charges = (
            Charge(
                id=DEMO_CHARGE_IDS[0],
                subscriber_id=primary.id,
                description="Smart 35 monthly renewal",
                amount=Decimal("35.00"),
                currency="GHS",
                charged_at=now - timedelta(days=22),
            ),
            Charge(
                id=DEMO_CHARGE_IDS[1],
                subscriber_id=primary.id,
                description="International roaming data pass",
                amount=Decimal("18.50"),
                currency="GHS",
                charged_at=now - timedelta(days=5),
            ),
            Charge(
                id=DEMO_CHARGE_IDS[2],
                subscriber_id=primary.id,
                description="Mobile money service fee",
                amount=Decimal("1.25"),
                currency="GHS",
                charged_at=now - timedelta(days=2),
            ),
            Charge(
                id=DEMO_CHARGE_IDS[3],
                subscriber_id=primary.id,
                description="Out-of-bundle voice",
                amount=Decimal("3.75"),
                currency="GHS",
                charged_at=now - timedelta(hours=19),
            ),
        )
...
async def demo_data_present(database: Database, subscriber_id: UUID) -> bool:
    async with database.session() as session:
        return (
            await session.exec(select(Subscriber.id).where(Subscriber.id == subscriber_id))
        ).first() is not None
